...
- データベースの利用
    - コントローラーからモデルを活用してアクセスする
        - コントローラー内で＠変数に対してテーブル情報を代入
        - View内で変数名.カラム名とする事で特定のデータを参照する事ができる
- 命名規則
    - モデルの命名規則
        - モデルはデータベースを作成するための設計図であり、設計図は一つであるため単数形で作成する。
    - テーブルの命名規則
        - テーブル内の情報はモデルからの情報が複数になるため複数形で作成する
    - Viewの命名規則
        - Viewフォルダは配下に複数のViewファイルを持つため複数形
    - contorollerは複数のアクションを持つため複数形で作成する
- データベースへの情報の保存方法（ユーザ体験）
...
要件
　パスワードが設定されていないユーザがある
　　rails consoleでパスワードを設定したい
ユーザ操作
　なし
処理
　コンソールを立ち上げる
　  　rails console
　  find_by メソッドを活用してユーザ情報を取得する
　  　user = User.find_by(id: 1)
　  取得したユーザのpasswordカラムに値を準備する
　  　user.password="入力内容”
　  準備した値をデータベースへ保存する
　  　user.save
　　エラー内容
　　　rails consoleを起動したのちにfind_by メソッドを活用してユーザ情報を取得しようとするとエラーが表示される
　　　　NameError: undefined local variable or method `ture' for User (call 'User.connection' to establish a connection):Class     from /usr/lib/ruby/gems/2.4.0/gems/activerecord-5.0.3/lib/active_record/dynamic_matchers.rb:21:in` method_missing'
仮説
　　　　　　問題点
　　　　　　　tureメソッドだと思っているがスペルに間違えがある事で正しく動作していない
　　　　　　対策
　　　　　　　スペルの修正を行う
　   　検証
　   　 スペルの修正をした結果正常に動作した。
