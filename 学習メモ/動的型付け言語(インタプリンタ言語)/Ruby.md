        ...
        -  **便利な範囲オブジェクトの使い方**
            ...
            - case文で使う
                - 範囲はcase文と組みあせて使用する事もできる
                    ...
                    ```
            - 値が連続する配列を作成する
                - 範囲オブジェクトに対してto_sメソッドを使用すると連続した値の配列を作成する事ができる
                    - [*範囲]の形でも記載する事ができる
                ```Ruby
                (1..5).to_a #=> [1, 2, 3, 4, 5]
                (1...5).to_a #=> [1, 2, 3, 4]
                ...
                - 繰り返し処理を行う場合
                    ...
                    sum #=> 10
                    ```
                    - stepメソッドを使って繰り返し処理をする間隔を指定する
                    ```Ruby
                    numbers = []
                    # 1から10まで2回ごとに繰り返し処理を行う
                    (1..10).step(2) { ¦n¦ numbers
                    ```
    - コンソール
        - なぜ入力待機になるのか？
            - getsメソッドで入力待機状態になる
        ...
        - 怠惰
            - テストの自動化
                - テスティングフレームワークを作成する事で自動で実行結果をチェックしてくれる
                    - 実行結果に間違えがあれば教えてくれる
        - ファイル管理
            - テスト部分のプログラムはプログラム本体と分離して管理する
                - プログラム本体 => lib/
                - テストコード => test/
                    - testコードには本体で記載しているメソッドなどを読み込まさせたい
                        - require_relative
                            - require_relative '../lib/fizz_buzz'
            - プログラム本体とテストプログラムを同一ファイルで記載する
                - ファイル名を変えてテストファイルを保存する
                    - テストファイルに本体部分のファイルを読み込む
        - テストコードを記載する幸せ
            - テストの自動化
                - テストを人がしなくてよくなる
                    - プログラムを記載する手間があるがプログラマの3大美徳を求める怠惰なプログラマを求める
            - テスティングフレームワークの種類
                - 標準インストール
                    - Minitest
                    - test-unit
                        - 最も歴史が深いテスティングフレームワーク
                            - Ruby本体のテストコードもtest-unitで記載されている
                - Gemでインストールが必要
                    - RSpec
                        - 独自のDSLがある
                            - クラスの定義やメソッドの定義が出てこない独特な記載方法をする
                                - 慣れると便利に記載する事がで切る
        - テスト駆動型の開発手順
            - ①テストコードを書く。
            - ②テストが失敗することを確認する。
            ...
            - ⑩テストがパスすることを確認する。
                ...
                - リファクタリングする
                    - 仮実装　＝＞　Fake it
                        - テスト対象のメソッドとテストコードがちゃんとリンクしているかチェックしている
            - 三角測量
                - 1辺の長さとその両端にある2角がわかる
                    - その交点から三角形の残りの1点を確定できる
    - Ruby特有のイディオム
        ...
        ↓結果を変数に代入しif文でnilではない事を確認している
        
        ```jsx
        def show_currency(country)
            # 条件分岐内で直接変数に代入してしまう（値が取得できれば真、できなければ偽）=> 変数への代入自体が戻り値をもつ
        ...
        ```
        - =と==を間違えられてしまう可能性
            - nilかもしれないオブジェクトを安全に呼び出すメソッド
                - &.演算子 => ぼっち演算子(lonely opertator)
                    - nilではない場合にはその結果を
                    - nilだった場合にはnilを返す
        ```Ruby
        def show_currency(country)
        ...
        - ||=を使った自己代入(nilガード)
            - limit ||= 10
                - limitがnil or falseだった場合には10を代入する
                    - それ以外の場合には自身をそのまま返す
            - デフォルトの値を求める計算が１行で終わらない場合にはbegin/endで囲む方法もある
        - !!を使った真偽値の型変換
            - Rubyにおいてnil,falseは偽、それ以外は全て真として扱われる
                - ?で終わるメソッドを定義した場合には確実にtreu,falseだけを返すようにしたい
                ```ruby
                def user_exists?
                    # データベースなどからユーザを探す（なければnil）
                    user = find_user
                ...
                ```
                - それ自身をtrue or falseに変換するためのコードになる
    - 正規表現　＝＞　regular expression => RubyのRegexpクラスの由来
        - 正規表現そのものについて
            - 正規表現は一種のミニ言語である
                - パターンを指定して文字列検索や置換を行う
        - 正規表現そのものについて
        - Rubyにおける正規表現オブジェクト
        ...
        - ハッシュ記法を変換するプログラム
            - ハッシュの記載方法は２通りある
                - どちらも有名な記載方法なので問題ない
                    - しかし、ソースコード上は統一する方が望ましい
            - 今回はハッシュを=>ではなく右側にコロンをつける方法に統一させる
                - さらに name => :kouki や、 name=>:kouki など、前後のスペースがバラバラになっている事に注意する必要がある
                    - 変換後のスペースに関しては統一できるようにする
- 概念
    - プログラムの本質
        - データと処理（コンピュータへの命令）
            - 処理は川の流れに例えられる
