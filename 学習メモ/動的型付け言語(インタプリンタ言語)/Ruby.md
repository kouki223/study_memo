        ...
        - 怠惰
            - テストの自動化
                - テスティングフレームワークを作成する事で自動で実行結果をチェックしてくれる
                    - 実行結果に間違えがあれば教えてくれる
        - Minitestの基本
            - テストの自動化にはテスト用のフレームワークを使用する
                - Minitest
                    - Rubyのインストールと共にインストールされる
                    - 学習コストが比較的少ない
                    - railsデフォルトのテスティングフレームワーク
                        - rails開発時に知識を活かしやすい
            - 手順
                1. フレームワークのルールに沿ってRubyプログラムを記載する
                2. 1.で作成したテストコードを実行する
                3. テスティングフレームワークが実行結果を報告する
        - Rubyプログラムを記載する    
            - テストコードの雛形
                ´´´Ruby
                ...
                - require 'minitest/autorun'
                    - ライブラリの読み込み
                - 1505~1511行目 => テストの本体部分
                    - SampleTest => クラス名
                        - 命名は自由
                            - 習慣的にTestで終わる、Testで始まる名前をつける事が多い
                        - テストのファイル名はテストのクラス名と合わせる
                            - JapaneseCalendarTest => japanese_calendar_test.rb
                            - TestOrderItem => test_order_item.rb
                                - クラス名：パスカルケース
                                - ファイル名：スネークケース
                    - Minitest::Testクラスを継承する
                    - メソッド部分
                        - Minitestはtest_で始まるメソッドを探して実行する
                            - クラス内で複数のtestを定義してもよい
                        - test_sample
                            - Minittestが提供するassert_equalメソッド
                                - 、'ruby'.upcaseの実行結果が'RUBY'になることを検証している
            - Minitestが提供するメソッドは多く存在するが、今回は３つ使用する
                - # aがbと等しければパスする
                    - assert_equal b, a
                - # aが真であればパスする
                    - assert a
                - # aが偽であればパスする
                    - refute a
        - テストの実行
            - テストの実行には先ほど、テストとして定義したファイルを実行する
                - #Runningの後にテストの進行度合いが.で表示される
                    - テストの実行が完了した場合にはFinishedと実行結果が出力される
                    - テストに失敗した場合
                        - 進行状況の部分に.ではなく失敗(Failure)の意味であるFが表示されるようになる
                    - 最後に表示される実行結果のまとめで失敗を報告する
            - テストを実行し検証前にエラーが起きる場合
                - 進行状況の部分にE(Error)が表示される
                    - エラーが起きるとそのテストメソッドは終了し次のメソッドへ移行するようになる
- 概念
    - プログラムの本質
        - データと処理（コンピュータへの命令）
            - 処理は川の流れに例えられる
